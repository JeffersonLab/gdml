# function to check whether a test file exists
AC_DEFUN( GAC_CHECK_TEST_FILE, [

if test -z $1; then
  AC_MSG_ERROR([function requires an argument of file to test]);
else
  arg_test_file=$1
fi
...
# function to check a package directory with an (optional) test file
#
# args:
# $1 = dir to check
# $2 = pkg name
# $3 = test file relative to $1
AC_DEFUN( GAC_CHECK_PKG_DIR, [
...
if test -n ${arg_test_file}; then
  GAC_CHECK_TEST_FILE( [${arg_dir}/${arg_test_file}] )
fi
